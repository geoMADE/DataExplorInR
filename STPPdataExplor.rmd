...
$$\lambda(s,t)=m(s)\mu(t)$$
where $m$ is a fixed non-negative function and $\mu$ is a non-negative preditable process. If this holds, we call the process separable with respect to the mark $s$. 
...
data <- read.table(file="E:/UNIVERSITY OF LEEDS SUBMISSIONS/synthesised data/rmarkdown/chicago_burglary.csv", sep=",",head=T)
...
??^(s, t) = m(s)?(t).The top right panel of Figure 3 shows comparison between Kb(u, v)?????u2v
...
further analyzed in M?ller and Ghorbani (2012).
